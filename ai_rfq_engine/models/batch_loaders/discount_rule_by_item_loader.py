...
class DiscountRuleByItemLoader(SafeDataLoader):
    ...
    def __init__(self, logger=None, cache_enabled=True, **kwargs):
        ...
        if self.cache_enabled:
            ...
            cache_meta = Config.get_cache_entity_config().get("discount_rule")
    ...
    def batch_load_fn(self, keys: List[str]) -> Promise:
        from ..discount_rule import get_discount_rules_by_item
        ...
        for item_uuid in uncached_keys:
            try:
                rules = get_discount_rules_by_item(item_uuid)
                if self.cache_enabled:
                    self.set_cache_data((item_uuid), rules)
